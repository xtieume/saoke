...
def extract_table_and_merge_words(pdf_path, column_boundaries, y_tolerance=5, x_tolerance=10, line_y_tolerance=10):
    csv_output_path = 'extracted_table_data.csv'
    ...
    with pdfplumber.open(pdf_path) as pdf, open(csv_output_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for page_number, page in enumerate(pdf.pages):
            ...
            print(f"Đang xử lý trang {page_number + 1} trên tổng số {len(pdf.pages)}")

            # Comment phần tạo hình ảnh để tăng tốc
            # page_image = page.to_image(resolution=300)
            
            # Comment phần vẽ các đường kẻ lên hình ảnh
            ...
            # Comment phần lưu ảnh để tăng tốc
...
# Gọi hàm để trích xuất bảng từ PDF và xuất ra CSV
x_tolerance = 200
y_tolerance = 5
line_y_tolerance = 5
